#!/usr/bin/env python
"""
run_probe_sweep_ltfv6_grouped_cv.py

Same probe sweep as run_probe_sweep_ltfv6.py (the R²(dist)/R²(log)/R²(bins)/
Acc(bins) headline numbers), but CV folds are grouped by source log (parsed
from h5["sample_path"]) so frames from the same continuous drive never
appear in both train and test.

Why: navtest has only 23 distinct logs. Plain frame-shuffled KFold (what
run_probe_sweep_ltfv6.py uses) leaks log identity across the split -- this
is the same leakage already found and fixed for the extended labels in
probe_extended_labels_ltfv6_grouped_cv.py (e.g. has_vehicle_front balanced
accuracy dropped from 0.92 to 0.83 once grouped). The primary dist_nearest_*
sweep here was never re-checked under grouped CV until now.

Usage:
    python run_probe_sweep_ltfv6_grouped_cv.py
    python run_probe_sweep_ltfv6_grouped_cv.py --label dist_nearest_any
    python run_probe_sweep_ltfv6_grouped_cv.py --h5 /path/to/latents.h5
"""
...
OUT_DEFAULT = "/jet/home/jjain2/Interpretable_Control/probe_results_ltfv6_grouped_cv.json"
...
def probe_per_bin(X, y_dist, y_bins, groups):
    """Same as run_probe_sweep_ltfv6.py's probe_per_bin, but folds are
    grouped by log and a bin is skipped if it doesn't have >=2 distinct
    logs (can't generalize across drives with only one drive present)."""
    results = {}
    for b, (name, width) in enumerate(zip(BIN_NAMES, BIN_WIDTHS)):
        ...
        gkf_b   = GroupKFold(n_splits=n_folds)
        pipe    = Pipeline([("scale", StandardScaler()), ("ridge", RidgeCV(alphas=ALPHAS))])
        mae = -cross_val_score(pipe, X_b, y_b, cv=gkf_b, groups=g_b, scoring="neg_mean_absolute_error")
        r2  =  cross_val_score(pipe, X_b, y_b, cv=gkf_b, groups=g_b, scoring="r2")
...
def probe_layer(X, y_dist, y_log, y_bins, gkf, groups):
    def reg():
    ...
    r2_dist = cross_val_score(reg(), X, y_dist,              cv=gkf, groups=groups, scoring="r2")
    r2_log  = cross_val_score(reg(), X, y_log,               cv=gkf, groups=groups, scoring="r2")
    r2_bins = cross_val_score(reg(), X, y_bins.astype(float), cv=gkf, groups=groups, scoring="r2")
    acc     = cross_val_score(clf(), X, y_bins,              cv=gkf, groups=groups, scoring="accuracy")
    return {
...
def main():
    ...
    print(f"Distinct logs : {n_groups_total}")
    ...
    with h5py.File(args.h5, "r") as h5:
        for key in HOOK_KEYS:
            ...
            r = probe_layer(X, y, y_log, y_bins, gkf, groups)
            results[key] = r
            print(f"  {key:<56} "

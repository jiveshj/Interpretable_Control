#!/usr/bin/env python
"""
probe_extended_labels_ltfv6_grouped_cv.py

Same probe sweep as probe_extended_labels_ltfv6.py, but CV folds are grouped
by source log (parsed from h5["sample_path"]) so that frames from the same
continuous drive never appear in both train and test.

Why: the navtest cache has only 23 distinct logs contributing 19-546 frames
each. Plain frame-shuffled KFold (used in probe_extended_labels_ltfv6.py)
leaks log identity across the split -- a zero-activation "predict the mean
of other frames in this log" baseline alone scores R²=0.13 under shuffled
KFold vs R²=-0.03 under log-grouped CV. GroupKFold/StratifiedGroupKFold here
keep every log entirely in train or entirely in test, giving an honest
estimate of generalization to driving scenes the probe has never seen.
...
Usage:
    python probe_extended_labels_ltfv6_grouped_cv.py
    python probe_extended_labels_ltfv6_grouped_cv.py --h5 /path/to/latents.h5
"""
...
OUT_DEFAULT = "/jet/home/jjain2/Interpretable_Control/probe_extended_results_ltfv6_grouped_cv.json"
...
def log_id_from_path(p):
    """Parse the source-log directory name out of a sample_path entry.

    sample_path looks like:
      .../navtest/<log_dir>/<scenario_token>/transfuser_feature.gz
    <log_dir> (e.g. "2021.06.28.18.03.27_veh-14_00620_01581") identifies one
    continuous drive; many scenario_token subfolders/frames share it.
    """
...
def load_groups(h5):
    paths = h5["sample_path"][:]
    return np.array([log_id_from_path(p) for p in paths])
...
def probe_regression(X, y, cv, groups):
    r2  = cross_val_score(reg_pipe(), X, y, cv=cv, groups=groups, scoring="r2")
    mae = -cross_val_score(reg_pipe(), X, y, cv=cv, groups=groups, scoring="neg_mean_absolute_error")
    return {
...
def probe_classification(X, y, cv, groups):
    acc  = cross_val_score(clf_pipe(), X, y, cv=cv, groups=groups, scoring="accuracy")
    bacc = cross_val_score(clf_pipe(), X, y, cv=cv, groups=groups, scoring="balanced_accuracy")
...
def probe_per_bin(X, y_dist, y_bins, groups):
    ...
    for b, (name, width) in enumerate(zip(BIN_NAMES, BIN_WIDTHS)):
        mask  = y_bins == b
        n_bin = int(mask.sum())
        n_groups_bin = int(len(np.unique(groups[mask]))) if n_bin else 0
        if n_bin < 2 or n_groups_bin < 2:
            results[name] = {"n": n_bin, "n_groups": n_groups_bin, "note": "too few samples/groups"}
            continue
        X_b, y_b, g_b = X[mask], y_dist[mask], groups[mask]
        n_folds  = min(N_SPLITS, n_groups_bin)
        kf_b     = GroupKFold(n_splits=n_folds)
        pipe     = Pipeline([("scale", StandardScaler()), ("ridge", RidgeCV(alphas=ALPHAS))])
        mae = -cross_val_score(pipe, X_b, y_b, cv=kf_b, groups=g_b, scoring="neg_mean_absolute_error")
        r2  =  cross_val_score(pipe, X_b, y_b, cv=kf_b, groups=g_b, scoring="r2")
...
def _print_per_bin_tables(label_name, per_bin_rows, y_dist, y_bins):
    ...
    for key, pb in per_bin_rows:
        ...
        for name in BIN_NAMES:
            d = pb.get(name, {})
            cols.append(f"{'too few':^{COL_W}}" if "note" in d
...
def run_dist_label(label_name, y_raw, h5, full_groups):
    ...
    groups = full_groups[valid]
    y_bins = np.clip(np.digitize(y, BIN_EDGES) - 1, 0, len(BIN_NAMES) - 1).astype(int)
    n_valid = int(valid.sum())
    n_groups = int(len(np.unique(groups)))

    print(f"\n{'='*65}")
    print(f"Label: {label_name}  (n={n_valid}, n_logs={n_groups})")
    ...
    for key in HOOK_KEYS:
        ...
        r  = probe_regression(X, y, kf, groups)
        pb = probe_per_bin(X, y, y_bins, groups)
    ...
    return {"type": "dist_reg", "n_valid": n_valid, "n_groups": n_groups,
            "bin_counts": counts.tolist(), "layers": layer_results}


def run_label(label_name, y_raw, h5, is_clf, full_groups):
    ...
    for key in HOOK_KEYS:
        ...
        if is_clf:
            r    = probe_classification(X, y, cv, groups)
            ...
            r    = probe_regression(X, y, cv, groups)
    ...
    return {"type": "clf" if is_clf else "reg", "n_valid": n_valid, "n_groups": n_groups,
...
def main():
    ...
    with h5py.File(args.h5, "r") as h5:
        full_groups = load_groups(h5)
        print(f"Parsed {len(full_groups)} sample_paths into {len(np.unique(full_groups))} distinct logs")

        for label_name in DIST_REG_LABELS:
            if label_name not in labels_npz:
                print(f"[SKIP] {label_name} not in npz")
                continue
            all_results[label_name] = run_dist_label(label_name, labels_npz[label_name], h5, full_groups)

        for label_name in OTHER_REG_LABELS:
            if label_name not in labels_npz:
                print(f"[SKIP] {label_name} not in npz")
                continue
            all_results[label_name] = run_label(label_name, labels_npz[label_name], h5, is_clf=False, full_groups=full_groups)

        for label_name in CLF_LABELS:
            if label_name not in labels_npz:
                print(f"[SKIP] {label_name} not in npz")
                continue
            all_results[label_name] = run_label(label_name, labels_npz[label_name], h5, is_clf=True, full_groups=full_groups)

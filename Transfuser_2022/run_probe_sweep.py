...
def main():
    ...
    with open(out_dir / "probe_results.csv", "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=[
            "name", "dim", "r2_exact", "r2_log", "r2_bins", "acc_bins", "gap", "mae"
        ])
        writer.writeheader()
        writer.writerows(summary["layers"])

    print(f"  CSV saved → {out_dir}/probe_results.csv")


    # ── 6. Figures ───────────────────────────────────────────────────────────
    if args.save_figures:
        save_all_figures(results, gt_distance, gt_bins, output_dir="figuresV3")
    else:
